...
// node_modules/@vidstack/react/dist/dev/chunks/vidstack-LxLCezlI.js
...
var VimeoProvider = class extends EmbedProvider {
  ...
  _watchVideoInfo() {
    ...
    getVimeoVideoInfo(videoId, abort).then((info) => {
      ...
      promise.reject();
      {
        (_a = this._ctx.logger) == null ? void 0 : _a.warnGroup(`Failed to fetch vimeo video info for id \`${videoId}\`.`).labelledLog("Error", e).dispatch();
      }
    });
    return () => {
      promise.reject();
  ...
  _onTimeUpdate(time, trigger) {
    if (this._skipTimeUpdates && time === 0)
      return;
  ...
  _onLoaded(trigger) {
    var _a;
    const videoId = this._videoId();
    (_a = this._videoInfoPromise) == null ? void 0 : _a.promise.then((info) => {
      if (!info)
        return;
  ...
  _onReady(duration, trigger) {
    const { $iosControls } = this._ctx, { controls } = this._ctx.$state, showEmbedControls = controls() || $iosControls();
  ...
  _onPause(trigger) {
    ...
    (_a = this._pausePromise) == null ? void 0 : _a.resolve();
    this._pausePromise = null;
  }
  _onPlay(trigger) {
    var _a;
    ...
    (_a = this._playPromise) == null ? void 0 : _a.resolve();
    this._playPromise = null;
  }
  ...
  _onChaptersChange(chapters) {
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      track.addCue(
        new window.VTTCue(
          chapter.startTime,
          (nextChapter == null ? void 0 : nextChapter.startTime) ?? realDuration(),
          chapter.title
        )
      );
    }
  ...
  _onError(error, trigger) {
    ...
    if (error.method === "play") {
      (_a = this._playPromise) == null ? void 0 : _a.reject(error.message);
      return;
    }
    {
      (_b = this._ctx.logger) == null ? void 0 : _b.errorGroup(`[vimeo]: ${error.message}`).labelledLog("Error", error).labelledLog("Provider", this).labelledLog("Event", trigger).dispatch();
    }
  }
...
//# sourceMappingURL=vidstack-LxLCezlI-3CCUF6FD.js.map
