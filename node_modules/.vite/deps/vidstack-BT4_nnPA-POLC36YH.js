...
} from "./chunk-TJSBMHSG.js";
import {
  RAFLoop
} from "./chunk-TPNDJWMC.js";
import {
  DOMEvent,
  ListSymbol,
  TimeRange,
...
// node_modules/@vidstack/react/dist/dev/chunks/vidstack-BT4_nnPA.js
...
var REMOTE_TRACK_TEXT_TYPE = chrome.cast.media.TrackType.TEXT;
var REMOTE_TRACK_AUDIO_TYPE = chrome.cast.media.TrackType.AUDIO;
var GoogleCastTracksManager = class {
  ...
  _getRemoteTracks(type) {
    var _a;
    const tracks = ((_a = this._cast.mediaInfo) == null ? void 0 : _a.tracks) ?? [];
  ...
  _syncLocalTracks() {
    ...
    for (const localTrack of localTextTracks) {
      ...
      if (!hasRemoteTrack) {
        untrack(() => {
          var _a;
          return (_a = this._onNewLocalTracks) == null ? void 0 : _a.call(this);
        });
        break;
      }
    }
  ...
  _syncRemoteActiveIds(event) {
    ...
    this._editTracksInfo(editRequest).catch((error) => {
      var _a;
      {
        (_a = this._ctx.logger) == null ? void 0 : _a.errorGroup("[vidstack] failed to edit cast tracks info").labelledLog("Edit Request", editRequest).labelledLog("Error", error).dispatch();
      }
    });
  }
...
var GoogleCastProvider = class {
  ...
  async play() {
    ...
    (_a = this._player.controller) == null ? void 0 : _a.playOrPause();
  }
  async pause() {
    var _a;
  ...
  setMuted(muted) {
    ...
    if (hasChanged)
      (_a = this._player.controller) == null ? void 0 : _a.muteOrUnmute();
  }
  setCurrentTime(time) {
    var _a;
    this._player.currentTime = time;
    this._notify("seeking", time);
    (_a = this._player.controller) == null ? void 0 : _a.seek();
  }
  setVolume(volume) {
    var _a;
    this._player.volumeLevel = volume;
    (_a = this._player.controller) == null ? void 0 : _a.setVolumeLevel();
  }
  async loadSource(src) {
    var _a;
    if (((_a = this._reloadInfo) == null ? void 0 : _a.src) !== src)
      this._reloadInfo = null;
  ...
  _resumeSession() {
    ...
    const { muted, volume, remotePlaybackInfo } = this._ctx.$state, localState = remotePlaybackInfo();
    this.setCurrentTime(
      Math.max(this._player.currentTime, ((_a = localState == null ? void 0 : localState.savedState) == null ? void 0 : _a.currentTime) ?? 0)
    );
    this.setMuted(muted());
    this.setVolume(volume());
    if (((_b = localState == null ? void 0 : localState.savedState) == null ? void 0 : _b.paused) === false)
      this.play();
  }
  _endSession() {
  ...
  _getStreamType() {
    var _a;
    const streamType = (_a = this._player.mediaInfo) == null ? void 0 : _a.streamType;
  ...
  _buildLoadRequest(src) {
    var _a, _b, _c, _d;
    const mediaInfo = this._buildMediaInfo(src), request = new chrome.cast.media.LoadRequest(mediaInfo), info = this._ctx.$state.remotePlaybackInfo();
    request.autoplay = (((_a = this._reloadInfo) == null ? void 0 : _a.paused) ?? ((_b = info == null ? void 0 : info.savedState) == null ? void 0 : _b.paused)) === false;
    request.currentTime = ((_c = this._reloadInfo) == null ? void 0 : _c.time) ?? ((_d = info == null ? void 0 : info.savedState) == null ? void 0 : _d.currentTime) ?? 0;
    return request;
  }
  ...
  _onNewLocalTracks() {
    this._reload(this._player.isPaused, this._player.currentTime).catch((error) => {
      var _a;
      {
        (_a = this._ctx.logger) == null ? void 0 : _a.errorGroup("[vidstack] cast failed to load new local tracks").labelledLog("Error", error).dispatch();
      }
    });
  }
};
...
//# sourceMappingURL=vidstack-BT4_nnPA-POLC36YH.js.map
