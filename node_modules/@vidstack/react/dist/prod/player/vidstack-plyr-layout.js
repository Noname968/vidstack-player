...
function PlyrPreviewScrubbing() {
  const $src = useMediaState("source"), { thumbnails, previewTime } = usePlyrLayoutContext(), $previewTime = useSignal(previewTime), $RemotionThumbnail = useSignal(RemotionThumbnail), $hasRemotionThumbnail = $RemotionThumbnail && isRemotionSource($src);
...
function PlyrPoster() {
  const $src = useMediaState("source"), $poster = useMediaState("poster"), { posterFrame } = usePlyrLayoutContext(), $RemotionPoster = useSignal(RemotionPoster), $hasRemotionPoster = $RemotionPoster && isRemotionSource($src) && isNumber(posterFrame);
  return slot(
    "poster",
...
function PlyrDownloadButton() {
  const { download } = usePlyrLayoutContext(), $source = useMediaState("source"), url = isString(download) ? download : download?.url, filename = (!isString(download) ? download?.filename : null) || "media", downloadText = usePlyrLayoutWord("Download");
  return slot(
    "download",
    url || isAudioSrc($source) || isVideoSrc($source) ? /* @__PURE__ */ React.createElement(
      "a",
      {
        className: "plyr__controls__item plyr__control",
